...
public class Java8BasicQuestion {
    ...
    static void sortMapBasedOnValue() {
        ...
        Map<String, Integer> sortedMapAsc = unsortedMap.entrySet().stream().sorted(Map.Entry.comparingByValue())
    ...
    static void printListOfUniqueHobbie(List<Student> studentList){
        ...
        List<String> uniqueHobbie=studentList.stream()
                .flatMap(student -> student.getHobbiesList().stream())
                .map(Hobbies::getHobbieName).distinct().collect(Collectors.toList());
